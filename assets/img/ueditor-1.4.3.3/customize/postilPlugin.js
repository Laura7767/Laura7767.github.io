			...
			if(eval('/'+tag+'/ig').test( node.tagName ) && node.getAttribute('plugins') == 'postil'){
				...
				if (author == name) {
					var html = popup.formatHtml(
						'<div style="width: 400px;"><div style="line-height: 24px; border-bottom: 1px solid #e3e4e6; font-weight: bold">'+date+'&nbsp;'+author+': </div><p style="line-height: 24px;word-break: break-all;max-height: 300px;overflow-y: auto;">'+content+'</p><div style="text-align:right;"><span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></div></div>'
					);
				} else {
					var html = popup.formatHtml(
						'<div style="width: 400px;"><div style="line-height: 24px; border-bottom: 1px solid #e3e4e6; font-weight: bold">'+date+'&nbsp;'+author+': </div><p style="line-height: 24px;word-break: break-all;max-height: 300px;overflow-y: auto;">'+content+'</p></div>'
					);
				}
